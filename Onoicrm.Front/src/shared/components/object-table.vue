<template>
  ...
  >
    ...
    <Column v-for="column in metaInfo.columns" :key="column.field" v-bind="column">
      <template #body="{ data }">
...
import {toFilterArray, toPascalCase} from "../lib/helpers";
import {TConfirm, defaultConfirm} from "../models/notification/TConfirm";
import {IMessage,defaultMessage} from "../models/notification/IMessage";
...
import {EntityMetaInfo} from "../models/entity-meta-info/EntityMetaInfo";
...
const items:ComputedRef<any[]> = computed(()=> dataSource.items.value.filter((i:any) => {
